...
export default function ReferencesPage() {
  ...
  return (
    <div className="min-h-screen text-stone-100 relative overflow-hidden bg-3franjas-light">
      ...
      <nav aria-label="controles" className="fixed top-4 right-5 z-50 flex items-center gap-3">
        <button
                  ...
                  <li key={r.name} className="grid grid-cols-[auto,1fr] gap-3">
                    ...
                    <p className="text-sm leading-relaxed">
                      ...
                      <span className="block text-xs mt-1 opacity-90">
                        {t.emailLabel}: <a className="underline" href={`mailto:${r.email}`}>{r.email}</a> | {t.cityLabel}: {r.city}
                      </span>
                    </p>
                  </li>
                ))}
      ...
      <nav className="fixed bottom-6 right-6 z-40 flex flex-col items-end gap-2">
        ...
        >
          ...
          <span className="hidden sm:inline text-sm font-medium">{t.prevLabel}</span>
        </Link>
        <Link
          href="/contacts"
          className="group relative flex items-center gap-3 rounded-full bg-amber-950 hover:bg-amber-900 px-3 py-2 text-white shadow"
        >
          <span className="hidden sm:inline text-sm font-medium">{t.nextLabel}</span>
